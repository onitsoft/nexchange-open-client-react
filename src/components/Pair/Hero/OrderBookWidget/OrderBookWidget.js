...
class OrderBookWidget extends Component {
  ...
  UNSAFE_componentWillUpdate(nextProps) {
    ...
    const pairChange =
      nextProps.selectedCoin.deposit !== this.props.selectedCoin.deposit ||
      nextProps.selectedCoin.receive !== this.props.selectedCoin.receive;
    const pricesFetched =
      nextProps.price.pair === `${nextProps.selectedCoin.receive}${nextProps.selectedCoin.deposit}` &&
      nextProps.price.receive > 0 && nextProps.price.deposit > 0;
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      (this.props.selectedCoin && this.props.selectedCoin.receive !== prevProps.selectedCoin.receive) ||
                ...
                {!myOrdersExpanded ? (
                  <div className={styles.widget}>
                    ...
                    <div className="col-xs-12 col-sm-12 col-md-6 col-lg-4">
                      ...
                      <ul className={`nav nav-tabs ${styles['tabs']}`}>
                        <li>
                          <a
                            ...
                            onClick={() => this.handleOrderBookOrderTypeChange('BUY')}
                          >
                            {t('orderbookwidget.buy')}
                          </a>
                        ...
                        <li>
                          <a
                            ...
                            onClick={() => this.handleOrderBookOrderTypeChange('SELL')}
                          >
                            {t('orderbookwidget.sell')}
                          </a>
                      ...
                      <div className="col-xs-12">
                        <button
                          className={`${styles.btn} ${order_type === 'BUY' ? styles['btn-buy'] : styles['btn-sell']} 
                        ...
                        >
                          {order_type === 'BUY'
                            ? `${t('orderbookwidget.buy')} ${this.props.selectedCoin.receive} ${t('orderbookwidget.with')} ${
                                this.props.selectedCoin.deposit
                              }`
                            : `${t('orderbookwidget.sell')} ${this.props.selectedCoin.receive} ${t('orderbookwidget.for')} ${
                                this.props.selectedCoin.deposit
                              }`}
                        </button>
                      </div>
                    ...
                    <OrderDepth />
                    <MyOrders expanded={false} expandMyOrders={this.expandMyOrders} collapseMyOrders={this.collapseMyOrders} />
                  </div>
                ) : (
                  <div className={styles.widget}>
                    <MyOrders expanded={true} expandMyOrders={this.expandMyOrders} collapseMyOrders={this.collapseMyOrders} />
                  </div>
                )}
              </div>
            </div>
...
const mapStateToProps = ({ order, selectedCoin, price, error, wallet, orderBook }) => ({
  order,
  selectedCoin,
  price,
  error,
...
export default connect(mapStateToProps, mapDispatchToProps)(OrderBookWidget);
