...
class QuestionAnswer extends Component {
  ...
  componentDidMount() {
    const browserPath = window.location.pathname.split('/')[2]
  ...
  setNegativeFeedback = () => {
    if (!this.state.negativeFeedback) {
      ...
      this.props.openSupportModal(this.props.question || this.props.t(`faq.${this.props.id}`));
    }
  };
    ...
    return (
      <div className={`question-answer ${styles.container}`} id={`faq.${this.props.id}`}>
        <div className={`${styles.question}`} onClick={this.onClick} data-test="question-opener">
          ...
          <h3>{this.props.question || this.props.t(`faq.${this.props.id}`)}</h3>
        </div>
        <div className={`${this.state.open ? `${styles.answer} ${styles.active}` : `${styles.answer}`}`}>
          ...
          <div className={styles.feedback}>
            ...
            <div className={styles.buttons}>
              <button
                className={`btn ${styles.positive} ${this.state.positiveFeedback ? styles.active : ''}`}
                onClick={() => {
              ...
              <button
                className={`btn ${styles.negative} ${this.state.negativeFeedback ? styles.active : ''}`}
                onClick={() => {
